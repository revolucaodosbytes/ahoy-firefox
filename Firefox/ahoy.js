var Ahoy = function() {
	...
	browser.storage.local.get(
		['sites_list', 'proxy_addr'],
		function(result) {
			if (result.sites_list !== undefined) this.sites_list = result.sites_list;
	...
	xhr.onreadystatechange = function() {
		if (xhr.readyState == 4) {
			...
			document.dispatchEvent(
				new CustomEvent('onSitesUpdated', {
					detail: {
						sites: resp
					}
				})
...
Ahoy.prototype.update_proxy = function(forceReload) {
	...
	xhr.onreadystatechange = function() {
		if (xhr.readyState == 4) {
			...
			// Dispatch the event
			document.dispatchEvent(
				new CustomEvent('onProxyUpdated', {
					detail: {
						proxy_addr: server,
...
Ahoy.prototype.update_callbacks = function() {
	...
	browser.tabs.onUpdated.removeListener(
		this.update_browse_action_icon.bind(this)
	);
...
Ahoy.prototype.update_browse_action_icon = function(
	tabId,
	changeInfo,
	tabInfo
) {
...
Ahoy.prototype.after_update = function(details) {
	...
	var current_version = parseVersionString(
		browser.runtime.getManifest().version
	);

	if (
		last_version.major != current_version.major ||
		last_version.minor != current_version.minor ||
		details.reason === 'install'
	) {
		browser.tabs.create({
			url: browser.extension.getURL('views/release_notes.html'),
			active: true
		});
	}
};
...
Ahoy.prototype.init_events = function() {
	...
	document.addEventListener(
		'onProxyUpdated',
		this.event_proxy_updated.bind(this),
		false
	);
	...
	document.addEventListener(
		'onSitesUpdated',
		this.event_sites_updated.bind(this),
		false
	);
};
...
Ahoy.prototype.event_sites_updated = function(e) {
	console.log(
		'[EVENT] Sites list updated. Total size: ' + e.detail.sites.length
	);
...
Ahoy.prototype.check_for_blocked_site = function(details) {
	...
	if (warning_ips.indexOf(details.ip) === -1) return;

	// Make sure we flag this as a redirect, if it's the case
	if (details.redirectUrl !== undefined) this.last_request_redirected = true;
